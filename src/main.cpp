...
uint256 hashGenesisBlock("0x9d18a7a306173dbd62ef579603c704b60859e0515a97c32f2e4d321b3815c785");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Jocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Jocoin Signed Message:\n";
...
{
    // Jocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 25 * COIN; //So coin nhan duoc moi block duoc tim thay

    // So coins trong mot block bi giam di mot nua khi 420000 blocks duoc tim ra, mat khoang 4 nam
    nSubsidy >>= (nHeight / 420000); // Jocoin: 420k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Jocoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // Jocoin: 2 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Jocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb1e82b4fad658cff0c83e102376145bb73b898044b96f98159476f8e31bc5a64");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "2/5/2024 JoiCoin, Jocoin will be successful in the future!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04748f2c4c99d0550cd6bc35e6a525b271da96fe86c38638312b252a2a7e6e723920a3117f4ae32e58316a232bd4a6d714ea5ca24fc2dd20b6c0a0ea1f1be75ab9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1714619447;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085851023;

        if (fTestNet)
        {
            block.nTime    = 1714619435;
            block.nNonce   = 387833510;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x214911235fcef0f5fe99b0f80a717718ccface11f49fe8e9680def6cf5bef30d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb3, 0xda }; // Jocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JocoinMiner(CWallet *pwallet)
{
    printf("JocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("jocoin-miner");
    ...
    try { loop {
        ...
        printf("Running JocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JocoinMiner, pwallet));
}
